...
class OnboardingActivity : AppCompatActivity() {
    ...
    private fun normalizeTimeOrNull(input: String): String? = try {
        val t = LocalTime.parse(input, flexibleTimeFmt)
        t.format(strictHHmm)
    ...
    private fun attachTimeMask(editText: EditText) {
        ...
        if (editText.hint.isNullOrBlank()) editText.hint = "예: 01:30"
    }
